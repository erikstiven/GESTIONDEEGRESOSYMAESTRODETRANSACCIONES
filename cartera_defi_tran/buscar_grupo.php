<?php
    ...
    $tabla = '';	
		$sql = "select tran_cod_tran, trans_tip_tran, tran_des_tran, tran_ant_tran, 
		trans_tip_comp, tran_cod_tret, tran_cod_cuen, tran_rem_tran,tran_prot_tran
				from saetran
    ...
    if($oIfx->Query($sql)){
    	if($oIfx->NumFilas() > 0){
    		do{
				...
				$tran_rem_tran  = $oIfx->f('tran_rem_tran');
				$tran_prot_tram  = $oIfx->f('tran_prot_tran');

                $tran_des_tran   = str_replace("'", " ", $tran_des_tran);
				$img = '<div align=\"center\"> <div class=\"btn btn-success btn-sm\" onclick=\"seleccionaItem(\'' . $tran_cod_tran . '\',\'' . $trans_tip_tran . '\',\'' . $tran_des_tran . '\',\'' . $tran_ant_tran . '\',\'' . $trans_tip_comp . '\', \'' . $tran_cod_tret . '\', \'' . $tran_cod_cuen . '\', \'' . $tran_rem_tran . '\', \'' . $tran_prot_tram . '\')\"><span class=\"glyphicon glyphicon-ok\"><span></div> </div>';
    			//echo $nomPais;exit;
				$tabla.='{
